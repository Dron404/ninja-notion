// services
export const API_HOST = "https://ninja-notion-api-production-9ed7.up.railway.app";
export const ROUT_USER = "user";
export const ROUT_LOGIN = "login";
export const ROUT_GETUSER = "getuser";
export const ROUT_SAVE_PAGES = "savePages";
export const ROUT_USER_ACTIVATION = "user/activation";
export const ROUT_ACTIVATION = "activation";
export const ROUT_RESET_PASSWORD = "resetPassword";

// avatar
export const AVATAR_SIZE_XXL = "90";
export const AVATAR_SIZE_L = "25";
export const AVATAR_SIZE_M = "15";
